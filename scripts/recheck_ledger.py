...
async def main() -> None:
    ...
    try:
        for r in rows:
            ...
            # ⚠ `since` ОКРУГЛЯЕТСЯ ВНИЗ ДО ГРАНИЦЫ БАРА. Binance отдаёт свечи с
            # openTime >= since, а бар, внутри которого открылась сделка, начался РАНЬШЕ неё —
            # без округления биржа его просто не вернёт, и никакая фильтрация на нашей стороне
            # его уже не вернёт тоже. Это вторая половина того же смещения, что ложно
            # опровергло TLMUSDT: одной правки фильтра не хватило, проверено прогоном.
            span_ms = tf_min * 60_000.0
            since = int(op.timestamp() * 1000 // span_ms * span_ms)
            ...
            try:
                ...
                continue
            # ⚠ ОКНО ВКЛЮЧАЕТ СВЕЧУ, СОДЕРЖАЩУЮ ОТКРЫТИЕ. Прежнее условие `op <= b[0]`
            # выбрасывало её: у свечи метка — время ОТКРЫТИЯ бара, и бар, внутри которого
            # сделка открылась, начался РАНЬШЕ неё. Смещение одностороннее — оно способно
            # только сфабриковать вердикт «цена недостижима», но не подтвердить достижимость.
            #
            # ЗАМЕР: так был ложно опровергнут TLMUSDT. Рынок торговал ровно по 0.002636 на
            # свече 12:10 (low == цене выхода), через 27.4 с после открытия позиции; проверка
            # по тикам (fetch_trades, 1054 сделки в окне) даёт первый принт по этой цене в
            # 12:10:50. Выбросив свечу 12:10, скрипт печатал зазор +0.04% и записывал
            # +223.06% в «опровергнутые» — это 60% всей заявленной суммы опровержения.
            span_ms = tf_min * 60_000.0
            bars = [
                b for b in bars
                if op.timestamp() * 1000 < b[0] + span_ms and b[0] <= cl.timestamp() * 1000
            ]
            if len(bars) < 2:
