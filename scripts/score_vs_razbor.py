...
# Поле нулевой модели = [самый дальний уровень эталона … цена] с запасом. Запас нужен, чтобы
# центр полосы мог встать и ЗА крайним уровнем — иначе нулевая модель искусственно поджата к
# эталону и завышает контроль. Величина калибруется требованием приёмки П-1 (обе подмены —
# пустая дальняя полоса и сжатие вдвое — обязаны давать Δпревышения <= 0), а не «на глаз».
_CTRL_MARGIN = 0.05
...
def _control_recall(
    ...
    Сохраняются число зон и ширина каждой; центры разбрасываются равномерно по полю, которое
    задают ЭТАЛОН И ЦЕНА.

    ⚠ ПОЛЕ БРАЛОСЬ ИЗ НАШЕЙ ЖЕ КАРТЫ — и это делало «превышение» неизмерительным. Огибающая
    ``min(lo)…max(hi)`` по нашим полосам означает, что модуль сам задаёт себе планку: добавил
    дальнюю полосу — растянул поле нулевой модели — случайные центры разъехались — контроль
    обвалился — «превышение» выросло. Метрика мерила ``Σширин / огибающая``, а не качество
    уровней.

    ЗАМЕР 2026-07-27 (боевой ``build_symbol_setups``, эталон 15 кейсов, кадры ``OHLCV_LIMIT``,
    менялась ТОЛЬКО геометрия, которую видит нулевая модель):

    | подмена | recall | превышение |
    |---|---|---|
    | одна ПУСТАЯ полоса на −86% от спота, не ловящая НИ ОДНОГО из 123 уровней | без изменений | **+12.3 п.п.** |
    | все полосы сжаты вдвое (ноль новой информации) | **−2 уровня** | **+3.6 п.п.** |
    | выброшена полоса, ловившая 2 уровня автора | **−2 уровня** | **+5.1 п.п.** |

    То есть прибор НАГРАЖДАЛ за мусор и за потерю настоящих уровней. Для масштаба: лучший из
    ~30 перебранных кандидатов правки стоил +3.0 п.п. — то есть проигрывал бессмысленному
    сужению и вчетверо проигрывал пустой полосе. Ужесточение допуска не лечит (+10.2 п.п. даже
    на ``--tol 0.3``).

    Теперь поле задаётся снаружи модуля: от самого дальнего уровня ЭТАЛОНА и текущей цены, с
    полем ``_CTRL_MARGIN``. Карта на него не влияет никак, поэтому «нарисовать себе превышение»
    больше нечем. Критерий приёмки самой этой правки — обе величины выше обязаны стать ≤ 0.
    """
    bands = [(lo, hi) for lo, hi, _p, t in zones if "линия" not in t and hi > lo > 0]
    if not bands or not levels:
        return 0.0
    lv = [float(x) for x, _n in levels]
    span_lo = min(min(lv), price) * (1.0 - _CTRL_MARGIN)
    span_hi = max(max(lv), price) * (1.0 + _CTRL_MARGIN)
    if span_hi <= span_lo:
        return 0.0
    # Полоса шире всего поля центр разместить не даст — зажимаем, иначе uniform() получит
    # перевёрнутый интервал и молча вернёт значение вне поля (I-6: тихая подмена вместо отказа).
    bands = [(lo, hi) for lo, hi in bands if (hi - lo) < (span_hi - span_lo)]
    if not bands:
        return 0.0
...
async def main() -> None:
    ...
    if tot_lvl:
        ...
        if CONTROL:
            ...
            print(f"\n{'=' * 62}")
            print(f"ВЕРДИКТ: RECALL {tot_hit}/{tot_lvl} = {real:.1f}%   (допуск --tol {args.tol})")
            print(f"справочно: случайный контроль {ctrl:.1f}%, превышение {real - ctrl:+.1f} п.п. "
                  f"({_CONTROL_DRAWS} наборов/кейс)")
            # ⚠ ВЕРДИКТ ПЕРЕНЕСЁН НА RECALL, ПРЕВЫШЕНИЕ ОСТАВЛЕНО СПРАВОЧНО. Прежняя строка
            # гласила «значение имеет только превышение» — и это оказалось неверно.
            #
            # Аудит 2026-07-27 (карта не менялась, подменялась только геометрия, которую видит
            # нулевая модель): сжатие ВСЕХ полос вдвое — ноль новой информации — покупало
            # +3.6 п.п. превышения, ТЕРЯЯ 2 уровня автора; удаление полосы, ловившей 2 уровня,
            # давало +5.1 п.п. Худший случай был у самореферентной огибающей: одна ПУСТАЯ полоса
            # на −86% от спота, не ловящая ни одного из 123 уровней, покупала +12.3 п.п.
            #
            # Огибающая починена (см. :func:`_control_recall`) и эксплойт «дальняя пустая полоса»
            # закрыт: Δ +12.3 → −1.4. Но эффект сужения ОСТАЛСЯ (+3.6 → +7.1) и остаться должен:
            # это честный размен точности на полноту, а не дефект. Именно поэтому вердикт на
            # превышение вешать нельзя — величина растёт и когда карта стала лучше, и когда она
            # просто растеряла уровни. RECALL ведёт себя правильно в обоих случаях: у сужения он
            # строго падает (108 → 106), у пустой полосы не двигается вовсе.
            #
            # ⚠ И читать его надо на СТРОГОМ допуске. При --tol 1.0 recall насыщен: 108/123 у
            # половины проверенных арок, то есть «recall не изменился» — свойство шкалы, а не
            # правки. Ненасыщенные точки: 85/123 при --tol 0.5 и 73/123 при --tol 0.3.
            if args.tol >= 1.0:
                print("⚠ допуск 1.0 НАСЫЩЕН (108/123 у половины арок) — гонять также "
                      "`--tol 0.5` и `--tol 0.3`, вердикт читать по ним")
            print("Для ориентира: Osler (2000) намерила у профессиональных уровней превышение "
                  "над\nслучайными 4.6 п.п. (60.8% против 56.2%).")
            print("=" * 62)
