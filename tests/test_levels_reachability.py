...
EXPECTED_UNREACHABLE = {
    # ВТОРАЯ точка входа: `python -m hunt_core.engine BTC/USDT:USDT`. Недостижима от `watch` не по
    # ошибке — у неё своя задача: доказать на живых данных, что WS ДВИГАЕТ кадр, а не отдаёт
    # замёрзший REST-сид (`ws_advanced`). Ни один тест и ни один `verify_*` этого не покрывают:
    # `test_closed_bar_convention` пиннит соглашение «-1 = свежий закрытый бар», а не факт
    # продвижения. Замёрзший кадр — самый дорогой класс инцидентов проекта (`stale-htf-cache-trap`).
    #
    # ⚠ Честная оговорка: до 2026-07-26 «отдельная точка входа by design» держалась ровно на одной
    # строке — докстроке самого файла. Ни docs/, ни CLAUDE.md, ни README, ни CI её не упоминали,
    # поэтому при чистке она и выглядела мёртвой. Прогнан вживую (работает), внесён в CLAUDE.md
    # § Commands. Инструмент без читателя от мёртвого кода неотличим — читатель добавлен.
