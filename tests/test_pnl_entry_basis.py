...
Замер (пересчитан 2026-07-27 по ОЧИЩЕННОМУ леджеру — 283 настоящие записи): 40.6% зон
вырождены (нулевая ширина), там разницы нет. У остальных 168 медиана ширины **2.245%**,
p90 4.883%, максимум 13.26% — то есть до **6.63%** фантомной прибыли на сделку, больше, чем
весь буфер стопа.

⚠ Прежняя редакция называла 3682 записи и «88.5% вырожденных» — считалось по файлу, где
3423 строки были тестовыми фикстурами с `entry_lo == entry_hi` (утечка archive=True). Они и
раздували долю вырожденных. На настоящих данных зона НЕ вырождена у 59% сделок, а медианная
ширина вдвое больше — то есть дефект был не мельче заявленного, а КРУПНЕЕ.
...
def test_degenerate_zone_is_unaffected() -> None:
    """У 40.6% записей зона нулевой ширины — поведение обязано остаться прежним."""
