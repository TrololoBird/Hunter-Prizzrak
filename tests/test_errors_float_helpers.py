...
def test_require_finite_float_accepts_numeric_types():
    assert require_finite_float(1, "open") == 1.0
    assert require_finite_float(1.5, "open") == 1.5
    assert require_finite_float("2.25", "open") == 2.25
    assert require_finite_float(-3, "open") == -3.0
    assert require_finite_float(0, "open") == 0.0
...
def test_optional_finite_float_returns_float_for_numeric():
    ...
    assert optional_finite_float("2.5") == 2.5
    assert optional_finite_float(3.5) == 3.5


def test_optional_finite_float_delegates_to_finite_float_or_none():
    assert optional_finite_float(7) == finite_float_or_none(7)
...
def test_finite_float_or_none_numeric_str():
    assert finite_float_or_none("4.5") == 4.5
    assert finite_float_or_none("10") == 10.0
    assert finite_float_or_none("-2.25") == -2.25
...
def test_as_float_numeric():
    ...
    assert as_float("3.75") == 3.75
    assert as_float(math.inf) == 0.0
    assert as_float(math.nan) == 0.0
...
def test_row_float_default_for_non_dict():
    assert row_float(None, "price") == 0.0
    assert row_float(None, "price", default=1.0) == 1.0
    assert row_float("not-a-dict", "price") == 0.0
    assert row_float(object(), "price") == 0.0


def test_row_float_dict_lookup_coercion():
    row = {"price": "12.5", "vol": 3, "bad": "nope", "missing": None}
    assert row_float(row, "price") == 12.5
    assert row_float(row, "vol") == 3.0
    assert row_float(row, "bad") == 0.0
    assert row_float(row, "missing") == 0.0
    assert row_float(row, "absent", default=5.0) == 5.0
