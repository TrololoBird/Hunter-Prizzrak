...
Ф1 (стр.19): 3+ касания с проколами → стоп ВСЕГДА за wick-прокол (ext_lo/ext_hi).
Ф2 (стр.19): стоповый объём / лой ТФ-1 в 2-5% за границей → стоп прятать за них.
...
Ф6 (стр.60): вымпел — вход на 6-м касании, стоп за всю структуру.
"""
...
def test_stop_behind_wick_prokol_on_3plus_touches() -> None:
    """Курс стр.19: «если на 3+ касаниях были проколы за границы — стоп ВСЕГДА за
...
def test_stop_hides_behind_tf1_low_in_2_5pct_band() -> None:
    """Курс стр.19: лой ТФ-1 в 2-5% за границей → стоп прятать за него."""
...
def test_pennant_6touch_candidate() -> None:
    """Курс стр.60: «не успели от уровня → вход на 6-м касании», стоп за ВСЮ структуру
