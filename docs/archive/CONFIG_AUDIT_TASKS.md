...
> **АРХИВ** (2026-07-12). Задачи закрытого аудита конфигов. Мёртвые секции `[gate.*]` и устаревшие env-переменные с тех пор удалены. Живая ловушка (документированный ключ проигрывает хардкод-фоллбэку) описана в `CLAUDE.md` §Config; проверять агентом `config-drift-auditor`.
>
