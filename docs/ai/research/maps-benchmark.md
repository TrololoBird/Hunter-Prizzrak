...
> **Статус: ИСТОРИЧЕСКИЙ ЗАМЕР** (2026-07-14), сверено 2026-07-26. Три из семи путей
> (`market/{client,cross,streams}.py`) удалены 2026-07-19 — источники данных для карт
> переехали в `hunt_core/engine/`. Числа замера не перепроверялись после переезда:
> опираться на них нельзя, повторять замер — на живых данных.
