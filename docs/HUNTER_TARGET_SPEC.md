...
> **Статус: ЧАСТИЧНО УСТАРЕЛО** · последняя правка 2026-07-15, сверено 2026-07-26.
> **§1.3 (глобальные инварианты I-1..I-6) — АКТУАЛЬНО** и остаётся каноном; на него ссылается
> `CLAUDE.md`. **§2 (целевые контракты модулей) — УСТАРЕЛО**: §2.1 описывает `market/` как
> data-plane, а транспорт переехал в `hunt_core/engine/` 2026-07-19 (`5ba0fea`).
> Разделы про формат сигналов сверять с `prizrak/format_post.py` — карточка переписана
> 2026-07-22/25. I-7 (окно без замера) и I-8 (ссылка без символа) здесь ОТСУТСТВУЮТ — они
> живут в `CLAUDE.md`.
>
> ⚠ **2026-07-31: всё про сканер и бэктест-гейт в этом файле мертво.** Модуль МАНИПУЛЯЦИИ
> вырезан, `research/backtest_scanner.py` удалён — там, где спека требует «обязательный
> before/after бэктест на изменение детекции», такого гейта больше не существует, и
> единственная проверка эмиссии — живые данные. Инвариант **I-1 («Deep и Scanner не
> импортируют друг друга») выполняется вырожденно**: второго модуля нет.

Назначение: **целевая спецификация**, к которой код сходится **гейтнутыми правками**
(измерение на живых данных + сверка с методом на каждую), НЕ greenfield-rewrite и НЕ
архитектура из статей. Для вопросов «что система ДОЛЖНА гарантировать» приоритет у этого
файла; для вопросов «как устроен рантайм сегодня» — **у кода и `CLAUDE.md`**
(`docs/ARCHITECTURE.md` тоже пролежал устаревшим 10 дней и north-star'ом больше не зовётся).
