...
def _followup(
    ...
    return HuntFollowUp(
        ...
        payload={
            ...
            # ⚠ `poc` ОБЯЗАТЕЛЕН ЗДЕСЬ, и до 2026-08-01 его не передавали. Гейт `_handoff`
            # ниже считает R:R С якорем на ПОК, а это сообщение — БЕЗ него, то есть от
            # другой цены входа. Читатель видел в канале одно число, а решение принималось
            # по другому: замер дал 1.15 против 1.41 на лонге и 1.67 против 2.81 на шорте,
            # и всегда в занижение — сделка выглядела хуже, чем её оценил модуль.
            #
            # Докстрока самой `_rr_worst_fill` требует ровно обратного: «риск считается от
            # ТОЙ ЖЕ цены, которую карточка называет входом». Путь сообщения это нарушал.
            "rr": _rr_worst_fill(
