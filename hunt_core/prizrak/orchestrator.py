...
def _zone_edge_band(z: dict[str, Any], *, side: str) -> dict[str, Any] | None:
    ...
    if not (float(hi) > float(lo) > 0):
        return None
    # ⚠ ГРАНИЧНЫЙ КЛАСТЕР ИЗ ОДНОГО КАСАНИЯ НЕ ОБРАЗУЕТ ПОЛОСЫ. Это структурный факт, а не
    # порог: полоса here — расстояние между СРЕДНИМ кластера и его же ЭКСТРЕМУМОМ фитиля, а у
    # единственного элемента среднее, минимум и максимум тождественны. Докстрока выше это уже
    # фиксировала («ширина ровно 0 ГАРАНТИРОВАНА ПОСТРОЕНИЕМ»), но отказ стоял только на строго
    # вырожденном `hi <= lo`, поэтому полосы 0.001–0.1% проходили дальше.
    #
    # Пол ширины сюда по-прежнему НЕ вводится — прежняя редакция справедливо возражала, что
    # константу пришлось бы выбирать и калибровать (I-7). Константа и не нужна: достаточно
    # потребовать, чтобы у кромки было из чего складываться. Одно касание — это уровень, но не
    # зона; у автора все 12 названных зон стоят на нескольких касаниях.
    #
    # Замер 2026-07-28, живой реестр 54 зон: 63% уже минимальной ширины автора (0.78%), медиана
    # 0.466% против его 1.20%, и 5 зон уже 0.05% — включая опубликованный в канал
    # `шорт 64525.0–64525.1`, полосу в 10 центов на 64-тысячном активе.
    if int(touches or 0) < 2:
        return None
...
def _base_summary(
    *,
    ...
    return {
        ...
        "gates_failed": [],
        # ⚠ ИМЯ ЛЖЁТ: это не измеренная уверенность в геометрии, а ПРИОР по виду сетапа.
        # Все записи в поле — литералы: 0.7 здесь, 0.7/0.6 по виду ПП (`_pp_candidate`),
        # ×0.8 и ×0.75 на спекулятивных путях, +0.05 за подтверждённую фигуру (`figures.py`).
        # Ничто в модуле его не вычисляет. Механизм при этом рабочий и осмысленный — приор,
        # который темперируется, — поэтому он оставлен; переименование сломало бы чтение
        # исторических строк, где ключ уже записан. Но читать его как замер НЕЛЬЗЯ: он
        # двигает ранжирование сигналов (`engines/signal_queue.py` — ±0.03 к очку). (2026-07-26)
...
def _level_already_worked(
    ...
    """Есть ли у ``level`` прошлая отработка (курс стр.31: уровень, уже давший хорошую реакцию,
    слабее — «лимитными ордерами больше не торгуем»).

    Отработка = бар коснулся уровня (в пределах 0.6%) и цена затем ушла в благоприятную сторону
    на >= 2.5% в течение нескольких следующих баров. Последние ``exclude_last`` баров исключены —
    это ТЕКУЩИЙ тест, а не прошлый.

    ⚠ ИМЯ ВОЗВРАЩАЕМОГО ЧИСЛА БЫЛО ЛОЖЬЮ (исправлено 2026-07-27, I-6 «name-lie»). Прежняя строка
    обещала «number of prior worked reactions». Возвращается же **число КАСАВШИХСЯ БАРОВ**, за
    каждым из которых был благоприятный ход: окна ``scan[i+1:i+6]`` у соседних касаний
    пересекаются, поэтому ОДИН ход засчитывается многократно. Замерено: ETH 1883 → ``worked=3``
    от ОДНОГО хода 5.29%; ARB 0.0756 → ``worked=10`` при 4 раздельных реакциях.

    Практического вреда сегодня нет: ВСЕ потребители порогуют на ``>= 1`` (``setups._course_flags``
    и ``_zone_edge_candidate``/``_zone_candidate`` ниже), а при этом пороге величина эквивалентна
    честному «была хотя бы одна реакция», и ни один потребитель число не печатает. Но **числом
    реакций оно не является, и опираться на порог ``>= N`` при N>1 нельзя** без правила слияния
    пересекающихся окон.

    Переписывать арифметику намеренно НЕ стали: правка наблюдаемо no-op (0 изменённых вердиктов
    на 338 и на 250 живых ступенях), то есть непроверяема на живых данных — единственным
    способом, который в этом проекте засчитывается, — и вносит новую калибровочную константу
    (зазор слияния), для которой замера нет: доля ``worked>=2`` при зазоре 1/2/4/8/20 =
    21.2/14.0/8.8/6.0/3.6%, провала в этом ряду нет.
    """
...
def _forward_zone_candidate(
    *,
    ...
    def _score(zone: dict[str, Any], dist_pct: float) -> float:
        ...
        """
        # ⚠ RECENCY УБРАН ИЗ СИЛЫ УРОВНЯ. Было `0.3 + 0.7 * recency` — штраф до 3.3× за возраст,
        # то есть структура, которую цена трогала только что, считалась самой сильной. Курс
        # утверждает ровно обратное по обоим пунктам:
        #   • сила определяется ТФ и ОБЪЁМОМ (стр.22), возраст в перечне не участвует;
        #   • уровень умирает ОТРАБОТКОЙ, а не временем — «отработан на 1 касание … этот уровень
        #     становится больше не актуальным, мы его удаляем» (стр.25).
        # Свежесть касания не делает уровень сильнее; она означала лишь, что цена ещё внутри
        # структуры, — а такие теперь отсекает гейт `exited` (стр.23), и различать этим
        # множителем стало нечего: до сюда доходят только завершённые структуры.
        #
        # Отсечение прошлого режима остаётся за окном обзора (`_SETUP_LOOKBACK`, своё на каждый
        # ТФ) — там оно и должно жить, а не в множителе силы.
        distance_factor = 1.0 + dist_pct / 10.0
        return float(zone.get("zone_volume") or 0.0) / distance_factor
