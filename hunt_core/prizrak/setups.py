...
def _horizon_zones(
    ...
    #
    # Замер (I-7), `scripts/score_vs_razbor.py`, 15 кейсов двух авторов, кадры продовой глубины
    # (`OHLCV_LIMIT`), 2026-07-27: recall 107/123 → **108/123**, превышение над случайным
    # +50.1 → **+50.5 п.п.**, зон напечатано 976 → **976 (+0)**. Из всех перебранных вариантов
    # (окна, min_touches, max_width, _LADDER_MAX) это ЕДИНСТВЕННЫЙ, где превышение растёт, а
    # число зон не растёт вовсе. Ловится при этом «база-в-базе» — ПОК BCH 196, тот самый случай,
    # который докстрока ниже сама называет нерешённым.
    #
