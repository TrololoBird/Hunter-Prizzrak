...
class PrizrakConfig(BaseModel):
    ...
    # Потолок ДИСТАНЦИИ стопа от входа, %. У стопа не было ни волатильностного члена, ни клампа:
    # плоские 2% буфера ложились одинаково на 15-минутный скальп и на недельную зону, а сама
    # дистанция (вход→край структуры + буфер) не ограничивалась ничем — на максимально широком
    # боксе (``max_zone_width_pct``) стоп уезжал на ~14%.
    #
    # Это ровно тот отказ, который автор проговаривает вслух (BTC 1ч, 2026-07-25, 03:12): «тапнуть
    # чисто по рынку и поставить себе сразу свой стоп-лосс — он должен стоять за всей этой
    # структурой. А вся эта структура достаточно большая и стоп-лосс будет большой. Большого смысла
    # в этом нет.» То есть широкий структурный стоп — это ПРИЧИНА НЕ ВХОДИТЬ, а не просто число.
    #
    # Потолок не выдуман: это уже откалиброванная ширина зоны, по которой модуль решает, можно ли
    # ставить лимит (``_INTEREST_ZONE_MAX_WIDTH_PCT`` = 4%), плюс буфер за структуру (2%). Стоп
    # прячется за край структуры с запасом, значит его дистанция и складывается из этих двух.
    #
    # Замер на живых данных (2026-07-25, 10 символов, A/B на одном и том же снимке): гейт снимает
    # 3 сигнала из 8 и срабатывает на 9 символах из 10. Это не косметика — это самый частый
    # отказ после R:R, и он ловит ровно тот случай, когда широкий риск «оправдывался» дальней целью.
    max_stop_distance_pct: float = Field(default=6.0, ge=1.0, le=20.0)
