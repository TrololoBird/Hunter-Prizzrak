...
def _zone_volume(bars: list[dict[str, float]], first_idx: int, last_idx: int) -> float:
    ...
    * доля суммы, набранная барами, чей ``[low, high]`` вообще пересекает бокс, — **медиана
      38.8%**; у **66.2% зон больше половины «силы» набрано барами, на которых цена в боксе не
      была ни разу**;
    * ранговая ρ(``zone_volume``, длина пролёта) = **+1.000** (медиана внутри площадки),
      ρ(``zone_volume``, число касаний) = **0.37**.

    То есть ключ ранжирует ДЛИТЕЛЬНОСТЬ наторговки, а «сильнейшая база» ≈ «самая длинная»:
    выбранная база занимает 96–98% окна.

    ⚠ РАСХОЖДЕНИЕ ОСТАВЛЕНО ОТКРЫТЫМ НАМЕРЕННО, поведение НЕ менялось. Замерены и отвергнуты
    все пять кандидатов замены (объём/пролёт, VRVP-в-полосе, касания×объём, объём внутри полосы,
    пролёт с потолком) — каждый ломает хвост ТВХ, тот самый класс, что закрыт в `8347fd6`
    (например объём/пролёт: DEXE −0.74% → −21.45% от спота). Главное же в том, что **направление
    правки неизвестно**: никто не показал, что более короткая база держит ЛУЧШЕ длинной, а
    стр.22 вполне допускает обратное — длинная наторговка и есть сильнейшая. Пока нет парного
    touch-outcome замера по кандидатам внутри одной ячейки (в R, `track/equity.py::simulate_equity`,
    I-10), любая замена формулы — это выбор знака наугад.
