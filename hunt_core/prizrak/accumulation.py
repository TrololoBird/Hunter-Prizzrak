...
long-term traders still watch is legitimately far away and untouched for a while
(⚠ здесь стояла ЦИТАТА КУРСА «следующая сильная, непротестированная база» — её в курсе
НЕТ: свип по всем 69 страницам конспекта даёт 0 вхождений, снято 2026-07-27. Ближайшее
реальное высказывание принадлежит ДРУГОМУ автору и вторичному источнику:
``research/prizrak_corpus/prizrak_pol_matic.segments.jsonl`` — «какой-то структурный,
не протестированный, не расторгованный». Мысль верна, ссылка была выдумана) — recency is
not a "freshness"
...
def _zone_volume(bars: list[dict[str, float]], first_idx: int, last_idx: int) -> float:
    """Суммарный объём баров ПРОЛЁТА зоны — от первого касания до последнего.

    ⚠ ЭТО ЗАМЕР ДЛИТЕЛЬНОСТИ, А НЕ ОБЪЁМА НА УРОВНЕ, и цитата курса его НЕ обосновывает.
    Прежняя докстрока ссылалась на стр.22 («Сила уровня определяется ТФ и объёмом … смотрим по
    VRVP») — но VRVP это объём НА ЦЕНЕ, а здесь суммируются все бары подряд, без единого фильтра
    по цене. Замер 2026-07-27 на живых данных:

    * доля суммы, набранная барами, чей ``[low, high]`` вообще пересекает бокс, — **медиана
      36.4%** (два независимых замера дали 38.8% и 36.4%);
    * ранговая ρ(``zone_volume``, длина пролёта) = **+0.943**, ρ(``zone_volume``, число
      касаний) = **0.37**; пролёт выбранной базы занимает **82.7%** окна.

    То есть ключ ранжирует ДЛИТЕЛЬНОСТЬ наторговки, а «сильнейшая база» ≈ «самая длинная».
    (⚠ Прежняя редакция писала ρ = +1.000 и 96–98% окна — числа были завышены, вывод не меняется.)

    ⚠ **ЧТО ГОВОРИТ КУРС — исправлено 2026-07-27, прежняя редакция этой докстроки врала.** Здесь
    стояло «стр.22 вполне допускает обратное — длинная наторговка и есть сильнейшая». НЕ
    допускает: стр.22 меряет объём НА ЦЕНЕ, а не за время — «смотрим по VRVP», и стр.26 требует
    натягивать профиль ровно на свечи структуры. Длительности как шкалы силы в курсе нет вовсе.
    Более того, стр.22 прямо предпочитает ПЛОТНОЕ: «иногда в маленькой часовой наторговке может
    быть объем больше, чем в 4ч-1д накоплении — от таких структур стараемся брать». Так что по
    ИСТОЧНИКУ имя ключа лжёт, и это установлено.

    ⚠ **И ВСЁ РАВНО НЕ МЕНЯЕМ — потому что лечение измеримо хуже.** Замер 2026-07-27, парно на
    ОДНИХ кадрах, вердикт по recall против разметки двух авторов (прибор починен в тот же день):

    | ключ | --tol 0.5 | --tol 0.3 | зон | ρ с пролётом |
    |---|---|---|---|---|
    | текущий (сумма по пролёту) | **85/123** | **73/123** | 976 | +0.943 |
    | серия в боксе | 74/123 | 62/123 | 918 | −0.200 |
    | объём в полосе | 78/123 | 68/123 | 921 | +0.400 |
    | VRVP, взвеш. пересечением | 78/123 | 67/123 | 908 | +0.400 |

    Парно по уровням: кандидат теряет **17 и приобретает 6** (McNemar χ²=4.3). Теряются не
    случайные, а ЗАГОЛОВОЧНЫЕ уровни авторов — «🟢 ПОК структуры 62850», «🟢 верх лимиток
    (2-й ПОК) 63140», ETH 1771/1785/1817. Плюс откатывается хвост ТВХ в класс, закрытый
    коммитом `8347fd6`: DEXE −1.5% → **−21.4%**, KAITO −2.0% → −21.2%. `zone_watch` меняет
    карту у **25 символов из 25**.

    Итого: курс-верная величина проигрывает разметке самих авторов, и проигрывает трижды подряд
    (пять кандидатов раньше + три в этом замере). По иерархии истины обе стороны — источник №1,
    и решает замер.

    ⚠ Чего по-прежнему НЕ измерено: **удержание**. 75 символов, ~1640 сделок, две линейки — все
    ДИ накрывают ноль, знак переворачивается от набора контролей. И «импульсная реакция» стр.22
    (автор обещает плотным структурам РАЗМАХ, а не удержание) косвенно дала ПРОТИВОПОЛОЖНЫЙ знак
    (MFE Q1 5.78% против Q4 2.74%) при n=36–39 и перекрывающихся ДИ. Оба вопроса открыты.

    Читатели ключа: :func:`_zones_from_pivots` (бюджет ``max_zones``),
    ``setups._horizon_zones`` (выбор базы перезакупа), ``setups._rank_rungs`` (что публиковать).
    """
...
def find_accumulation_zones(
    ...
    *,
    ...
    # ⚠ «УРОВНЯ НЕТ, ПОКА ЦЕНА НЕ ВЫШЛА ИЗ СТРУКТУРЫ» — курс стр. 23, дословно:
    # «Пока цена не вышла из структуры - у нас нет уровня, уровень появляется когда цена
    # полноценно выходит из структуры на данном ТФ - тогда мы натягиваем профиль объема и
    # выделяем уровень. На тесте берем от него позицию.»
    #
    # До 2026-07-28 этого гейта не было НИГДЕ, а `recency` работал прямо наоборот: в
    # `orchestrator` вес считался как `0.3 + 0.7 * recency`, то есть структура, которую цена
    # трогала только что (и из которой, стало быть, ещё не вышла), получала МАКСИМАЛЬНУЮ силу.
    # Мы ранжировали выше всего ровно те объекты, которые методом уровнями ещё не являются.
    #
    # Это объясняет разом три расхождения с разбором автора (замер 2026-07-28): его лимитные
    # зоны стоят на 3–7% от цены, наши — на 0–2.6%; его ширины 0.78–2.67%, наша медиана 0.43%;
    # и его зон единицы на символ против наших 9–17. Всё это следствия одного: он публикует ПОК
    # ЗАВЕРШЁННЫХ структур, которые цена покинула и может прийти перетестировать, а мы —
    # текущие консолидации, внутри которых цена ещё сидит.
    #
    # Критерий выхода берётся готовый: `_open_test_start_idx` возвращает `None` ровно тогда,
    # когда последний бар ЗАКРЫЛСЯ вне полосы. Он и так работает по закрытиям, а не по фитилям,
    # что совпадает с курсом стр.24: «важно чтобы цена не закрывалась свечами за уровнем —
    # иначе это уже будет пробой».
    for z in zones:
        z["exited"] = _open_test_start_idx(bars, float(z["lo"]), float(z["hi"])) is None
    return zones
...
def _zones_from_pivots(
    ...
    *,
    ...
    # by traded VOLUME, not touch count. Volume ties break to the tighter box.
    #
    # ⚠ ССЫЛКА НА КУРС ИСПРАВЛЕНА 2026-07-27. Здесь стояло «the course explicitly prefers a
    # smaller-touch, higher-volume наторговка over a wider-touched one (стр.22)» — стр.22
    # КАСАНИЯ НЕ СРАВНИВАЕТ ВООБЩЕ. «4 и более точек» там — ценз существования базы, а
    # сравниваются «маленькая часовая наторговка» и «4ч-1д накопление», то есть классы ТФ:
    # «иногда в маленькой часовой наторговке может быть объем больше, чем в 4ч-1д накоплении —
    # от таких структур стараемся брать, ожидая импульсной реакции». Про касания — ни слова.
    #
    # Само ранжирование по объёму остаётся, и это решение ЗАМЕРА, а не цитаты: см. развёрнуто
    # в :func:`_zone_volume`.
