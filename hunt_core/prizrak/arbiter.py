...
def observe_outcome_gates(symbol: str, direction: str, *, now: datetime | None = None) -> None:
    ...
    Но подключить их «как есть» НЕЛЬЗЯ, и причина ПРОВЕРЕНА ЗАМЕРОМ, а не взята из прозы.

    Пороги ``SYMBOL_REPEAT_LOSER_NET_R = -3.0`` и ``-6.0`` откалиброваны, по комментарию
    рядом с ними, замером 2026-07-27 «по 283 настоящим записям». ЭТИХ ДАННЫХ В ДЕРЕВЕ
    БОЛЬШЕ НЕТ, и это факт, а не предположение — замер 2026-08-01:

        data/hunt_outcome_ledger.jsonl   5 строк
        data/signal_history.jsonl        4 строки
        записей с фазами манипуляций     0
        история леджера в git            отсутствует (данные не версионируются)

    То есть калибровка опирается на выборку, которую сегодня НЕЛЬЗЯ НИ ПОВТОРИТЬ, НИ
    ПРОВЕРИТЬ, а накопленного у призрака — пять записей. Включать по такой основе гейт,
    который останавливает эмиссию, значит управлять стратегией числом неизвестного
    происхождения.

    ⚠ ПРЕЖНЯЯ РЕДАКЦИЯ ЭТОГО АБЗАЦА УТВЕРЖДАЛА БОЛЬШЕ, ЧЕМ ПРОВЕРЕНО. Она говорила, что
    все 283 записи — «полоса МАНИПУЛЯЦИЙ», ссылаясь на докстроку
    `track/outcomes.py::lane_of`. Это цитата прозы, выданная за факт: модуль манипуляций
    вырезан (`a0773f2`), его данных в дереве нет, и состав той выборки сегодня
    неустановим. Владелец справедливо указал на противоречие — «где ты вообще нашёл
    манипуляции, ты же их сам вырезал». Правило проекта тут однозначно: утверждение о том,
    КАК СЕЙЧАС, из докстроки брать нельзя.
    ...
    if blocking:
        LOG.info(
            ...
            direction=direction,
            gates=blocking,
            note="НАБЛЮДЕНИЕ, эмиссия не остановлена: пороги откалиброваны на выборке, "
                 "которой в дереве больше нет (леджер 5 записей), проверить её нечем",
        )
