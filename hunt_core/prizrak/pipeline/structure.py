...
def _detect_structure(
    ...
    *,
    ...
    return {
        ...
        # Срез здесь снят 2026-07-26 — он резал ДО того, как потребитель отфильтрует сторону.
        #
        # Было: `_all_swing_highs[-6:]` с комментарием «nearest 6 above price». Но список
        # отсортирован ПО ВОЗРАСТАНИЮ (`sorted(set(...))` — по значению, не по расстоянию, вопреки
        # комментарию выше), поэтому `[-6:]` оставлял шесть самых ВЫСОКИХ, то есть самых ДАЛЁКИХ
        # хаёв. Ближайшее сопротивление над входом выпадало, `min_rr` в `_geometry_from_zone`
        # считался до более далёкой цели, и гейт пропускал сетап, чьё первое реальное препятствие
        # стоит прямо над входом. Зеркало для лоёв (reverse=True + `[:6]`) отбирало действительно
        # ближайшие — эта асимметрия и доказывала, что задумано было «ближайшие».
        #
        # Просто заменить на `[:6]` НЕЛЬЗЯ: список содержит пивоты по обе стороны цены, а сторону
        # отбирают уже потребители (`orchestrator::_structural_targets` — `level > entry`,
        # `deliver/confluence_grid` — `p > price`). Шесть самых низких хаёв этот фильтр выбросил бы
        # целиком, оставив пустую лестницу. Оба потребителя при этом сортируют ближайшими вперёд и
        # ограничивают счёт сами (`deduped[:max_levels]`, `sorted_lows[:6]`), поэтому верхний срез
        # был не нужен изначально — он только отнимал у них право выбора.
        "all_swing_highs": _all_swing_highs,
        "all_swing_lows": _all_swing_lows,
    }
