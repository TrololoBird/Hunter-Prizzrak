    ...
    for hz in (setups.get("horizons") or {}).values():
        ...
        for kind in ("perezakup", "dobor"):
            ...
            for z in zs:
                ...
                # −62.7%; COTI −42.0%; AKE −27.9%. Четыре символа из 22 глубже −15%, тогда как
                # самая глубокая зона, когда-либо опубликованная авторами, — ETH-сетка 1662
                # при цене 1927.52, это −13.8%. Такое «ТВХ» — не план, а контекст, и читателю
                # оно предлагается как точка входа.
                #
                ...
                # пропущено. Касания остались тай-брейком: на равном расстоянии сильнее та
                # зона, которую больше трогали.
                key = (hi, float(z.get("touches") or 0))
