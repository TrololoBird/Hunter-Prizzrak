...
# ⚠ `_DEFAULT_FUNDING_INTERVAL_H = 8.0` УДАЛЁН 2026-08-03 (T1.2). Он был единственным
# значением, которое когда-либо доходило до расчёта, потому что ключ `funding_interval_h`
# не писал никто. Замер живого `fetch_funding_intervals` по 743 символам Binance:
# 4 ч — 443 символа, 8 ч — 296, 1 ч — 4. Дефолт был неверен для 60% вселенной.
# Теперь интервал измеряется движком; неизмеренный интервал = отказ (`measured=False`),
# а не правдоподобная восьмёрка.
...
def _funding_pct(row: dict[str, Any], costs: CostModel) -> tuple[float, bool]:
    ...
    try:
        ...
        return 0.0, False
    # ⚠ ЗДЕСЬ БЫЛО `or _DEFAULT_FUNDING_INTERVAL_H`, И ЭТО ДВА ДЕФЕКТА В ОДНОЙ СТРОКЕ.
    #
    # 1. Ключ `funding_interval_h` НЕ ПИСАЛСЯ НИ ОДНОЙ СТРОКОЙ В ДЕРЕВЕ до 2026-08-03 —
    #    фантомная ручка, чтение всегда сваливалось в дефолт 8 ч. Замер живого
    #    `fetch_funding_intervals` по 743 символам Binance: **4 ч у 443, 8 ч у 296, 1 ч у 4**.
    #    То есть дефолт был неверен для 60% вселенной, и число интервалов удержания
    #    занижалось вдвое (на четырёх символах — восьмикратно). Издержка фандинга занижена
    #    ⇒ PnL в леджере завышен. Продюсер добавлен: engine → Derivs → market-блок.
    # 2. `or` вместо `is None`: легитимный ноль обнулился бы в 8.0. Здесь ноль невозможен
    #    физически, но идиома та же, что уже давала дефекты в этом файле, — не оставляю.
    #
    # Неизмеренный интервал теперь ОТКАЗ, а не восьмёрка: `measured=False` заставляет
    # вызывающего показать сумму как нижнюю оценку, вместо того чтобы выдать догадку
    # за расчёт. Молчаливая подстановка правдоподобного числа — ровно то, что запрещает I-6.
    interval_h = _num(market.get("funding_interval_h"))
    if interval_h is None or interval_h <= 0.0:
        return 0.0, False
