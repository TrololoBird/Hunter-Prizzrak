...
class PlaneCadence:
    ...
    **377.9 с** — все 17 наблюдённых обновлений подряд оказывались «протухшими» на здоровом
    ...
    ⚠ **Интервал и возраст — не одно и то же, и расходятся они закономерно.** Интервал ловит
    КАЖДЫЙ разрыв между обновлениями; возраст сэмплируется тиком раз в ~30 с и пики пропускает.
    Поэтому нормально видеть `bound_too_tight` по p90 интервала при нулевой доле строк за бондом
    по возрасту (замеры 2026-07-27: `kline.1m` p90 84 с при бонде 80 — возраст max 78.2 с из 400;
    `mark` p90 16 с при бонде 15 — возраст max 13.2 с из 200, 0.0% за бондом). Это не ложное
    срабатывание: бонд действительно впритык. Но и не повод менять константу — менять только по
    выборке в сотни интервалов, с новым замером рядом (I-7).
    """
...
class SymbolState:
    ...
    * **frame-backed** planes (``kline.<tf>``) keep a REST-seeded frame that WS merges into.
    ...
    def merge_frame(self, name: str, new_closed: list[Bar], stamp: PlaneStamp) -> None:
        """Слить свежезакрытые WS-бары в кадр: новые дописать, ПОВТОР ТОГО ЖЕ БАРА — заменить.

        ⚠ БЫЛО СТРОГО append-only (`if bar[0] > tail`), и это делало любую ошибку кадра
        ВЕЧНОЙ. Бар опознаётся по времени ОТКРЫТИЯ; финальная версия того же бара приходит с
        ТЕМ ЖЕ временем, то есть `bar[0] > tail` ложно — и обновление молча отбрасывалось.

        Почему это важно именно сейчас: до правки `rest.py::fetch_klines_full` сид приходил с
        формирующимся баром (замер: локальные часы опережают Binance на 43.4 с, бар отдавался
        на 16.6-й секунде жизни вместо 60-й). Такой бар несёт недоформированные объём и
        экстремумы — замер показал объём 3.71 против финальных 27.465, занижение в 7.4 раза, —
        и починить его было НЕЧЕМ: правильная версия отвергалась как «не новее».

        Источник течи закрыт, но обновление обязано доходить и без него: WS может прислать
        уточнённый бар, а рассинхрон часов может вернуться на другом хосте. Замена по
        совпадению времени открытия — это не «на всякий случай», а корректная семантика:
        одно время открытия = один бар, более поздние данные вернее.
        """
