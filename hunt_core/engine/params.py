...
# Документированный бюджет IP-веса Binance USDⓈ-M: 2400 за 1 минуту.
# ⚠ Это ЕДИНСТВЕННОЕ место, где число живёт как константа, а не как проза. До 2026-08-01
# оно повторялось в четырёх комментариях (`api.py`, `exchanges.py`, здесь, `rest.py`) и не
# читалось ни одной строкой кода — при том что ccxt отдаёт `last_response_headers`.
# Подтверждено живым `GET /fapi/v1/exchangeInfo` 2026-08-01: rateLimits содержит
# {REQUEST_WEIGHT, MINUTE, 1, 2400}. Биржа публикует бюджет машиночитаемо — при желании
# его можно не хардкодить, а брать оттуда; пока это TODO, а не сделанное.
WEIGHT_BUDGET_1M: int = 2400

# Доля бюджета, с которой начинаем предупреждать. Не «на глаз»: ccxt разрешает 1200
# cost-units/мин (rateLimit=50 мс, capacity=1 ⇒ burst отсутствует), то есть ПОЛОВИНУ
# биржевого. Значит штатный расход не может подойти к 1800 в принципе, и 75% — признак,
# что либо мимо троттлера ходит кто-то ещё, либо пора мерить, а не поднимать потолок.
WEIGHT_WARN_FRACTION: float = 0.75

# Сколько символов позиционирования обходить ОДНОВРЕМЕННО (`api.py::_poll_positioning`).
#
# ЗАМЕР 2026-07-31, живой прогон ступенями: последовательный обход стоил **40 с на символ**,
# то есть 280 с из такта 300 с уже на семи пиннутых; на 12 и 20 символах круг не закрывался
# за 450 с вообще. Разложение: обязательные паузы дают 6 × 1.2 = 7.2 с, остальные ~33 с —
# это RTT шести последовательных запросов (~5.5 с каждый на текущем канале).
#
# Почему именно 6. Ворота `_FD_GATE` (`rest.py::poll_futures_data`) разряжают СТАРТЫ на
# FUTURES_DATA_SPACING_S и отпускают лок до сетевого вызова, поэтому насытить их достаточно
# ceil(RTT / spacing) = ceil(5.5 / 1.2) ≈ 5 запросов «в полёте»; 6 даёт запас, а больше —
# просто очередь на воротах без выигрыша. Пол круга после правки: 6 запросов × N символов ×
# 1.2 с (7 символов ≈ 50 с, 20 ≈ 145 с), то есть узкое место переезжает с RTT на разрядку.
#
# ⚠ Поднимать выше без нового замера нельзя: у `/futures/data/*` отдельный бюджет
# 1000 запросов / 5 мин / IP, телеметрии веса он не отдаёт, и единственный сигнал перебора —
