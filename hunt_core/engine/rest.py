...
async def fetch_klines_full(
    ...
    *,
    ...
    if now_ms is not None:
        # Историческая выборка: вызывающий передал момент отсечки явно, ему и верим.
        return [b for b in out if int(b[0]) + interval_ms <= int(now_ms)]
    # ⚠ ЖИВОЙ ПУТЬ — ОТСЕЧКА ПОЗИЦИОННАЯ, БЕЗ ЧАСОВ.
    #
    # Здесь стояло `now = int(time.time() * 1000)` — ЛОКАЛЬНЫЕ часы. Правило «бар закрыт,
    # если T + D <= now» само по себе верное, но `now` брался у хоста, а хост может уходить.
    #
    # ЗАМЕР 2026-07-27: локальные часы опережают Binance на **43.4 секунды** (медиана 7
    # проб с компенсацией RTT, разброс 43.17–43.56 с). При таком сдвиге минутный бар
    # признаётся закрытым уже на 16.6-й секунде своей жизни вместо 60-й — то есть
    # формирующийся бар отдаётся **72% времени**. Проверено на живом вызове этой самой
    # функции: 5 символов из 5 получили бар возрастом 56.2 с из 60.
    #
    # Почему это дороже любой другой находки: нарушается I-5 в ИСТОЧНИКЕ. Форминг-бар несёт
    # недоформированные объём и экстремумы (замер: закрытие 64844.0/объём 3.71 против
    # финальных 64861.2/27.465 — объём занижен в 7.4 раза), и на нём считаются ВСЕ
    # индикаторы, профиль, пивоты и уровни. Ошибка не локализуется — она отравляет кадр.
    #
    # Binance для запроса «до сейчас» всегда возвращает формирующийся бар последним, поэтому
    # позиционный сброс эквивалентен фильтру по времени, но не зависит от часов вовсе. Это
    # ровно то, что делает `seed_ohlcv` (`bars[:-1]`) — в том же живом замере он утёк 0 из 14,
    # тогда как этот путь — 9 из 14.
    #
    # Цена: если последний бар всё же закрыт, мы теряем один бар. Отставание на один бар —
    # правильная плата за то, чтобы никогда не считать по недоформированному.
    return out[:-1]
...
async def fetch_ohlcv_between(
    ...
    if not bars:
        return []
    # ⚠ Локальные часы ОПЕРЕЖАЮТ (замер 2026-07-27: +43.4 с против Binance), поэтому
    # `min(end_ms, now_local)` расширяет окно вперёд и пускает формирующийся бар, когда
    # `end_ms` дотягивается до настоящего. Отступ на один интервал делает отсечку
