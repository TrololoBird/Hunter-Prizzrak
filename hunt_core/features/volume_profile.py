...
# ⚠ БЫЛО 20, И ЭТО РАСХОДИЛОСЬ С ОСТАЛЬНЫМ ПРОЕКТОМ. Одну и ту же величину — ПОК, якорь
# входа по методу («метка входа ставится точно на POC-линии», стр.21) — считали ТРИ пути
# с разным разрешением:
#
#     features/build.py::_build_vp   →  20 корзин  → карточка (`deliver/confluence_grid.py`)
#     maps/config.py::vp_buckets     →  60 корзин  → карта
#     prizrak/config.py::vp_buckets  →  60 корзин  → ЯКОРЬ ВХОДА
#
# То есть читатель видел в карточке ПОК, посчитанный не с тем разрешением, по которому
# ставится вход. Рядом в `maps/config.py` стоит чужой замер, объясняющий выбор 60:
# «24 корзины это ~$150 на корзину у BTC, слишком грубо, чтобы быть полезным» — при 20
# ещё грубее.
#
# ЗАМЕР РАСХОЖДЕНИЯ 2026-08-01 (5 символов × 1ч/15м, живые кадры): медиана **0.05%**,
# max 0.08% (BTC $49.7, ETH $1.49). Это одна двадцатая–одна шестидесятая стопа метода
# («за структуру с запасом 1–3%», стр.33), то есть на геометрию входа не влияет. Меняется
# не число, а СОГЛАСОВАННОСТЬ: три пути обязаны считать якорь одинаково.
#
# ЗАМЕР УСТОЙЧИВОСТИ (сдвиг окна на бар, 29 сдвигов на символ): медиана прыжка ПОК 0.000%
# у обоих разрешений; p90/max сопоставимы (BTC 0.668→0.786%, ETH 0.173→0.024%, SOL без
# изменений). Более мелкие корзины НЕ сделали якорь дёрганее — проверено, а не предположено.
VP_BUCKETS_DEFAULT = 60
VP_LOOKBACK_15M = 96
VP_LOOKBACK_1H = 48
