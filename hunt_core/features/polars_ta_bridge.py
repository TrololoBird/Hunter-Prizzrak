...
def stochastic_series(
    ...
    *,
    ...
    Сглаживания %K здесь НЕТ, и это осознанно названо: ``polars_ta.ta`` предоставляет только
    ``STOCHF``, а медленного ``STOCH`` в ней нет (проверено: ``hasattr(plta, "STOCH") is False``),
    поэтому %K вернётся сырым. Раньше сигнатура принимала ``smooth_k`` и молча его выбрасывала —
    вызывающий просил медленный стохастик и получал быстрый, не узнав об этом. Тот же класс дефекта,
    что и ложное имя ``buffer_pct``: параметр, который выглядит настройкой, но ею не является.
