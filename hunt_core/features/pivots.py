...
def _swing_points(
    ...
    *,
    ...
    ⚠ ФРАКТАЛ ЗДЕСЬ АСИММЕТРИЧНЫЙ: ``n`` баров СЛЕВА, ровно ОДИН справа. Это осознанно —
    подтверждение приходит на следующем закрытом баре, а не через ``n`` (`_swing_detect`
    ставит пивот на ``confirm_idx - 1`` и читает только уже закрытые бары, поэтому I-5 не
    нарушен: проверено 2026-07-27, новейший пивот всегда ≥1 бара от конца, а
    ``include_unconfirmed_tail=True`` в проде не зовётся ни разу).

    Но цена этого выбора не была записана, и её надо знать. Отраслевые реализации
    (TradingView, `lib_divergence`, LonesomeTheBlue, MQL5 Dashboard) используют СИММЕТРИЧНЫЙ
    ``n``/``n``, и наш детектор поэтому заметно менее избирателен. ЗАМЕР на BTC 1h, 1000 баров:

        n=2:  наших 355  против симметричных 280  = 1.27×
        n=3:  наших 304  против симметричных 216  = 1.41×
        n=5:  наших 223  против симметричных 124  = **1.80×**

    То есть утверждение аудита окон «фрактал поднят до отраслевого N=5» верно про ЛЕВУЮ
    сторону и неверно про правую: сравнивать частоту наших пивотов с этими реализациями
    напрямую нельзя, их вдвое меньше. Компромисс честный — мы платим избирательностью за
    скорость подтверждения, — но при калибровке порогов поверх пивотов это надо учитывать.
    """
    if work.is_empty():
        return (
...
def _pivot_rows(
    ...
    *,
    ...
    """Подтверждённые пивоты для дивергенций: ``[{idx, price, indicator}]``, не более 8 последних.

    ⚠️ ``max_lookback`` не откалиброван: пришёл первым же коммитом репозитория без замера.

    Замер 2026-07-26 (12 символов × 15m/1h/4h/1d, 48 срезов): разрыв между двумя ПОСЛЕДНИМИ
    пивотами — медиана 3–7 баров, p90 ≤ 16, максимум 16; НИ ОДИН не превысил 50. Подстановка
    30 / 100 / 200 вместо 50 не изменила ни одного флага дивергенции (0/48 в каждом случае).

    ⚠️⚠️ НО ФОРМУЛИРОВКА «НА ЖИВЫХ ДАННЫХ НЕ СРАБАТЫВАЕТ» БЫЛА НЕВЕРНА — исправлено
    2026-08-03. Она опиралась на разрыв между ДВУМЯ СОСЕДНИМИ пивотами, а фильтр ниже
    применяется к ВОСЬМИ возвращаемым, и это разные величины.

    Замер 2026-08-03 (15 символов × 15m/1h/4h/1d, 781 срез, 6245 пивотов):

        возраст ВСЕХ возвращаемых пивотов   медиана 37, p90 74, max 149
        возраст САМОГО СТАРОГО из восьми    медиана 68, p90 95, max 149

        сколько пивотов проходит фильтр:  30 → 41.2%   50 → 68.4%
                                         100 → 98.6%  200 → 100%

    То есть при ``max_lookback=50`` отсекается **31.6%** строк — фильтр работает. Он
    ВЫГЛЯДИТ инертным только потому, что единственный потребитель сравнивает лишь ДВА
    СОСЕДНИХ пивота, а их разрыв (≤16) в порог укладывается всегда: выброшенные шесть
    строк никто не читает. Отсюда и 0/48 изменений флага — результат верный, объяснение
    в прежней редакции было неверным.

    ⚠️ ПРАКТИЧЕСКОЕ СЛЕДСТВИЕ, а не терминологическая придирка: разрыв, отмеченный ниже
    («сравнивать текущий пивот с 10–16 предыдущими, как LonesomeTheBlue/TrendSpider»),
    при нынешнем значении 50 закрыть НЕЛЬЗЯ — медианный возраст восьмого пивота 68 баров,
    то есть порог молча обрежет сравнение до двух-трёх. Тот, кто возьмётся за ту правку,
    обязан поднять и это число, иначе получит тихо усечённый набор вместо ошибки.

    Настоящее поведение задают три другие вещи, и вот их и надо обсуждать:
      * фрактал ``_swing_points(n=2)`` — пивотом становится максимум 5-барного окна, поэтому пивоты
        идут каждые 3–7 баров;
      * срез ``[-8:]``;
      * сравниваются только ДВА последних пивота.

    Отсюда следствие, которое важнее самой константы: дивергенция меряется между соседними
    МИКРО-качелями, а не между видимыми свингами, как её читает трейдер. Формулы при этом верны
    (обычные и скрытые, обе стороны сверены с классическими определениями) — вопрос к объекту,
    к которому они применяются. Исправление (более широкий фрактал либо порог величины свинга)
    требует эталона по разборам автора; выдумывать число по одному наблюдению нельзя (§1 спеки).
    """
