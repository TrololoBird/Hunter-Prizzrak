...
def _next_bar_wake_ts(now: float, *, grid_s: float = _BAR_GRID_S, settle_s: float = _BAR_SETTLE_S) -> float:
    ...
    Args:
        now: Текущее epoch-время в СЕКУНДАХ, сведённое с биржей (``clock.now_ms()/1000``,
            не monotonic — сетка баров привязана к UTC). Прежняя редакция докстроки называла
            здесь ``time.time()``; вызывающий (см. ниже по файлу) читает часы из ``clock``.
