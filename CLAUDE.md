...
- **I-6. Fail-loud** — отсутствующие данные → явное «нет данных», **никогда сфабрикованное
  ...
  **25 подтверждено**, 15 закрыто в `fc84c76`/`df1ff75`/этом коммите.
  ...
  замер 2026-08-01 (`ruff --extend-select S110,S112`) нашёл **17 мест**: 11 в `hunt_core`
  (включая молчаливый отказ телеметрии в `bootstrap.py`, потерю кэша доминации и капитализации,
  и `detect_pereprior`, чей отказ лишал карточку авторского словаря слома) и 6 в `scripts/` —
  верификаторы молча пропускали пары символ/ТФ и печатали «нарушений не найдено» на неполном
  покрытии. Все закрыты, правило переведено из прозы в гейт: `S110`/`S112` в `extend-select`.
