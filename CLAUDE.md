...
- **I-6. Fail-loud** — отсутствующие данные → явное «нет данных», **никогда сфабрикованное
  ...
  **25 подтверждено**, 15 закрыто в `fc84c76`/`df1ff75`/этом коммите.
  ...
  Все закрыты, правило переведено из прозы в гейт: `S110`/`S112` в `extend-select`.
  ...
  Урок общий: **утверждение «класса больше нет» без механической проверки — это надежда.**
...
С 2026-08-01 ruff несёт ещё и **`S110`/`S112`** — это ГЛАВНАЯ директива владельца («молчаливые
ошибки НЕ ДОПУСТИМЫ»), переведённая из прозы в гейт. Срабатывание чинится строкой лога, а не
`noqa`: деградация, о которой не сказано, — та самая, что запрещена.
